...
locations of JSON manifest files. These keys are located in HKR device hives
...
In general, most applications should install layers into the `SOFTWARE\Khrosos\Vulkan`
paths, as these paths do not depend on any devices. The PnP registry locations are
intended specifically for layers that are distrubuted as part of a driver installation.
As such, they should not be used, except for layers that are tied to a device.
...
In order to find installed ICDs, the loader will scan through registry keys specific to Display
Adapters and all Software Components associated with these adapters for the
locations of JSON manifest files. These keys are located in HKR device hives
created during driver installation and contain configuration information
...
practical. That location clearly ties the ICD to a given device, and it is required
in newer versions of Windows. The `SOFTWARE\Khronos\Vulkan\Drivers` location is the
older method for locating ICDs, and is retained for backwards compatibility.
